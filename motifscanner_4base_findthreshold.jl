"""
This prints out the estimate score cuttoffs for a chosen p-value threshold. Use
usage: julia PROGRAM p-valuecutoff memefile positivefile DSnegativefile output"""
...
function load_motifs(filename)
    ...
    for m in keys(motifs)
		...
		tdict=Dict('A'=>Float64[],'C'=>Float64[],'G'=>Float64[],'T'=>Float64[])
...
function scoreCutoff_new(pwm,DsSeqs,pvaluecutoff,bg)
    """
    Calculate the scores for all of the kmers in the shuffled sequences, get the distribution then decide the score cutoff
    corresponding to the p-value cutoff
    """
...
function main()
    ...
    output = ARGS[5]
    #negativeregions="/Users/vungo/Downloads/Work/ENCFF001USR.narrowPeak.DS.faa"
    #positiveregions="/Users/vungo/Downloads/Work/ENCFF001USR.narrowPeak.faa"
    #output="/Users/vungo/Downloads/Work/ENCFF001USR.julia.scan.txt"
    ...
    for m in motifnames
        ...
        println("\n")
		write(target,"MOTIF\t"*m*"\t"*string(trainnegmaxscore)*"\t"*string(trainnegminscore)*"\t"*string(trainnegscorecutoff)*"\n")
  		"""      
